...
export default function ProfessionalCard(props) {
  ...
  const [classExpertise, setClassExpertise] = React.useState(
    props.professional.expertise.length > 1
      ? null
      : props.professional.expertise[0].id
  );
  ...
  const { data, isLoading } = useQuery(
    ["classes", { id: props.professional.id, day: calendarDay.toISOString() }],
    getProfessionalClassesOfDay,
    { enabled: isOpen }
  );
  const { data: classRating, isLoading: isLoadingRating } = useQuery(
    ["rating", { id: props.professional.id }],
    getProfessionalRating
  );
  const { user } = React.useContext(UserContext);
  let classTimes = [];
  let working_hours = JSON.parse(props.professional.working_hours);
  if (isSameDay(calendarDay, new Date())) {
    working_hours = working_hours.filter(
      (working_hour) => working_hour > new Date().getHours()
    );
  }
      ...
      <Flex bgColor="white" direction="column" p={2.5} borderRadius="md">
        <Flex>
          <Avatar
            ...
            src={props.professional.user.avatar_url}
            size="xl"
          />
        ...
        <Box ml="auto" mt="auto">
          <Button mr="2" colorScheme="blue" disabled={!user} onClick={onOpen}>
            Agendar
          </Button>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
                ...
                {isLoading ? (
                  ...
                  <Grid templateColumns="1fr 1fr" gap="1.5">
                    {working_hours.map((schedule_time) => {
                      const colorScheme =
                        getClassTimesButtonColor(schedule_time);
                      return (
                        <Button
                          key={schedule_time}
                          disabled={colorScheme === "red"}
                          colorScheme={colorScheme}
                          onClick={handleClassDateTimeChange}
                          value={schedule_time}
                        >
                          {schedule_time}:00
                        </Button>
          ...
          <ModalFooter>
            ...
            <Button
              colorScheme="blue"
              disabled={!!!classDateTime || !!!classExpertise}
